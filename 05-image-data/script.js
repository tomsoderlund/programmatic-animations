...
const drawAllTriangles = function (context) {
	...
	forEachTriangle(function (triangle, c, r) {
		...
		const colorValues = imageData.getPixel(c, Math.round(r*1.8));
		const RANDOM_VARIATION = 10;
		const randomColorValues = {
			r: colorValues.r + getRandomValue(-RANDOM_VARIATION, RANDOM_VARIATION),
			g: colorValues.g + getRandomValue(-RANDOM_VARIATION, RANDOM_VARIATION),
			b: colorValues.b + getRandomValue(-RANDOM_VARIATION, RANDOM_VARIATION),
		}
		const triangleColor = tinycolor(randomColorValues);
...
const initImageData = function (canvas, context) {
	imageData = new ImagePixels('girl.png', 12 / TRIANGLE_SIZE, function (property) {
		console.log(`imageData.getPixels():`, imageData.getPixel(0,0));
	});
};
